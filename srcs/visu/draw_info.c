...
/*   Updated: 2020/01/29 20:09:28 by yquaro           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
						...
						size_t players_counter)
{
	...
	if (vm.players[players_counter].is_alive)
	{
		...
		mvwprintw(win, (*line)++, 5 + ft_strlen("Player %d : "), \
			"%s        ", vm.players[players_counter].name);
	}
	else
	{
		wcolor_set(win, GRAY, NULL);
		mvwprintw(win, (*line)++, 5 + ft_strlen("Player %d : "), \
			"%s is DEAD", vm.players[players_counter].name);
	}
	wcolor_set(win, 0, NULL);
	mvwprintw(win, (*line)++, 7, "slogan: %s ", \
		vm.players[players_counter].comment);
	mvwprintw(win, (*line)++, 7, "last live: %zu ", \
		vm.players[players_counter].lives_last);
	mvwprintw(win, (*line)++, 7, "current live: %zu ", \
		vm.players[players_counter].lives_current);
}
...
void				draw_info(void)
{
	...
	line = 3;
	mvwprintw(vm.visu->win_info, line += 2, 5, "Cycles : %zu", \
		vm.cycles_all);
	mvwprintw(vm.visu->win_info, line += 2, 5, "Cycle to die : %zu", \
		vm.cycles_to_die);
	mvwprintw(vm.visu->win_info, line += 2, 5, "Checks : %zu", \
		vm.checks);
	mvwprintw(vm.visu->win_info, line += 2, 5, "Lives in round : %zu", \
		vm.lives_in_round);
	mvwprintw(vm.visu->win_info, line += 2, 5, "Number of cariages : %zu", \
		vm.process_count);
	line += 3;
