...
void	ldi_op(t_process *proc)
{
    ...
    if (DEBUG)
        ft_printf("proc id - %d: ldi op: arg1 - %d arg2 - %d arg3 - %d reg[%d] = %d\n",
                  proc->proc_id, proc->args_value[0], proc->args_value[1], proc->args_value[2], proc->args_value[2], proc->regs[proc->args_value[2] - 1]);
}
